    ...
    <section id="github" className="py-24 sm:py-32 bg-[#071A2B]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal className="text-center mb-16">
          ...
          <p className="text-[#8B9BB4] max-w-2xl mx-auto text-base sm:text-lg">Open-source projects and technical contributions.</p>
        ...
        <ScrollReveal className="max-w-3xl mx-auto mb-10">
          <div className="glass-subtle rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-5">
            <div className="w-16 h-16 rounded-full bg-[#7EB8DA]/10 flex items-center justify-center text-[#7EB8DA] flex-shrink-0">
              <Github className="w-8 h-8" />
            </div>
            <div className="text-center sm:text-left flex-1">
              <h3 className="font-semibold text-white text-lg">@{githubProfile.username}</h3>
              <p className="text-[#8B9BB4] text-sm">Data Science &amp; Machine Learning Projects</p>
            </div>
            <a href={githubProfile.url} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-[#071A2B] bg-[#7EB8DA] hover:bg-[#5BA3D0] rounded-xl transition-colors">
            ...
            <StaggerItem key={repo.name}>
              <a href={repo.url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2 text-[#7EB8DA] text-sm font-medium">
                    <GitFork className="w-4 h-4" /><span className="group-hover:underline">{repo.name}</span>
                  </div>
                  <ExternalLink className="w-3.5 h-3.5 text-[#5A7A94]" />
                </div>
                <p className="text-[#8B9BB4] text-sm leading-relaxed mb-3">{repo.description}</p>
                <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 text-xs text-[#8B9BB4] bg-white/5 rounded-md">{repo.language}</span>
                  <div className="flex items-center gap-1 text-[#5A7A94] text-xs">
